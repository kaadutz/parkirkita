...
<html lang="id">
<head>
    <meta charset="UTF-8"><title>Checkout Non-Member - ParkirKita</title>
    ...
    <style>
        ...
        #qr-reader { 
            width: 100%; 
            height: 300px; /* Tinggi fix agar kamera tidak gepeng */
            border-radius: 12px; 
            overflow: hidden; 
            position: relative;
            background: #000;
        ...
        #qr-reader video { 
            ...
            object-fit: cover !important; /* Memastikan video full cover */
            border-radius: 12px;
        }
    </style>
</head>
...
<div class="flex h-screen bg-slate-50 overflow-hidden">
    ...
    <div id="main-content" class="flex-1 flex flex-col overflow-hidden transition-all duration-400">
        <header class="flex-shrink-0 flex justify-between items-center p-4 bg-white border-b-2 border-slate-200 shadow-sm z-20">
             <div class="flex items-center"><button id="sidebar-toggle" class="text-gray-600 hover:text-[var(--brand-orange)] focus:outline-none mr-4 p-2 rounded-lg hover:bg-orange-50"><i class="fas fa-bars fa-lg"></i></button><h1 class="text-xl font-semibold text-slate-700">Checkout Non-Member</h1></div>
            ...
            <div class="container mx-auto max-w-6xl">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div class="lg:col-span-5 space-y-6">
                        <div class="content-card p-6">
                            <h2 class="text-xl font-bold text-slate-800 mb-2 flex items-center">
                                <i class="fas fa-ticket-alt text-orange-500 mr-2"></i> Scan Tiket Parkir
                            </h2>
                            <p class="text-slate-500 text-sm mb-4">Arahkan barcode tiket ke kamera atau gunakan alat scanner.</p>
                            
                            <div id="scanner-wrapper" class="relative">
                                <div id="qr-reader"></div> <div id="camera-placeholder" class="absolute inset-0 flex flex-col items-center justify-center bg-slate-100 rounded-xl">
                                    <i class="fas fa-camera text-4xl text-slate-400 mb-2"></i>
                                    <p class="text-sm text-slate-500">Kamera Nonaktif</p>
                                </div>
                            </div>
                            
                            <button id="toggle-camera" class="w-full mt-3 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg font-bold transition flex items-center justify-center gap-2">
                                <i class="fas fa-video"></i> Buka Kamera
                            </button>

                            <div class="mt-6 border-t pt-4">
                                <label for="parking_token" class="block text-sm font-bold text-slate-700 mb-2">Input Manual</label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <i class="fas fa-keyboard text-slate-400"></i>
                                    </div>
                                    <input type="text" id="parking_token" class="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all font-mono text-lg placeholder-slate-300" placeholder="Ketik token lalu Enter..." autofocus>
                    ...
                    <div class="lg:col-span-7">
                        <div id="empty-state" class="content-card p-10 text-center h-full flex flex-col items-center justify-center min-h-[400px]">
                            <div class="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                <i class="fas fa-file-invoice-dollar text-4xl text-slate-300"></i>
                            </div>
                            <h3 class="text-lg font-bold text-slate-400">Menunggu Scan Tiket</h3>
                            <p class="text-sm text-slate-400 mt-1">Detail pembayaran akan muncul di sini setelah tiket discan.</p>
                        </div>
                        ...
                        <div id="transaction-details" class="hidden space-y-6">
                            ...
                            <div class="content-card p-6 border-l-4 border-blue-500">
                                <div class="flex justify-between items-start mb-4 border-b pb-3">
                                    <div>
                                        <p class="text-xs text-slate-400 font-bold uppercase tracking-wider">Status Parkir</p>
                                        <h3 class="text-lg font-bold text-slate-800">Pengunjung Umum</h3>
                                    </div>
                                    <span id="display_token" class="text-sm font-mono bg-slate-100 px-3 py-1 rounded-full text-slate-600 font-bold tracking-widest">
                                        -
                                    </span>
                                </div>
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="border-t border-dashed border-slate-200 my-3 pt-2"></div>
                                    
                                    <div class="flex justify-between items-center">
                                        <span class="text-slate-600 font-bold text-lg">Total Tagihan</span>
                                        <span class="text-3xl font-bold text-orange-600" id="display_total_fee">Rp 0</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="content-card p-6">
                                <form id="checkout-form" action="proses_checkout.php" method="POST">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                        <div>
                                            <label for="license_plate" class="block text-sm font-bold text-slate-700 mb-1">Plat Nomor</label>
                                            <input type="text" name="license_plate" id="license_plate" class="w-full px-4 py-2 border border-slate-300 rounded-lg bg-slate-50 uppercase font-bold focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all" placeholder="B 1234 XYZ" required>
                                        </div>
                                        <div>
                                            <label for="vehicle_category" class="block text-sm font-bold text-slate-700 mb-1">Kategori</label>
                                    ...
                                    <div class="mb-6">
                                        ...
                                        <div class="mt-3 flex justify-between items-center px-1 bg-slate-50 p-3 rounded-lg border border-slate-100">
                                    ...
                                    <div class="flex justify-end pt-2">
                                        <button type="submit" id="submit-button" class="w-full md:w-auto bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-8 rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 transform hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none" disabled>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- LOGIKA HALAMAN INI ---
    const tokenInput = document.getElementById('parking_token');
    ...
    function showAlert(message, type = 'error') {
        ...
        alertContainer.innerHTML = `<div class="flex items-center gap-3"><i class="${icon.className}"></i><span class="font-medium">${message}</span></div><button onclick="this.parentElement.classList.add('hidden')" class="text-lg opacity-50 hover:opacity-100">&times;</button>`;
    ...
    function resetView() {
        ...
        changeDueDisplay.className = 'font-bold text-lg text-slate-400';
    ...
    async function processToken(token) {
        ...
        try {
            const response = await fetch(`api_get_transaksi.php?token=${encodeURIComponent(token)}&tipe=non_member`);
            const data = await response.json();

            if (data.success) {
                const tx = data.data;
                currentTotalFee = tx.total_fee;

                document.getElementById('transaction_id').value = tx.id;
                document.getElementById('display_token').textContent = token;
                document.getElementById('display_checkin').textContent = tx.check_in_formatted;
                document.getElementById('display_checkout').textContent = tx.current_time_formatted;
                document.getElementById('display_duration').textContent = tx.duration_formatted;
                document.getElementById('display_total_fee').textContent = `Rp ${currentTotalFee.toLocaleString('id-ID')}`;
                
                emptyState.classList.add('hidden');
                transactionDetails.classList.remove('hidden');
                alertContainer.classList.add('hidden');

                // Jika kamera nyala, matikan setelah sukses
    ...
    // --- KAMERA LOGIC (HTML5-QRCODE CLASS) ---
    function startCamera() {
        cameraPlaceholder.classList.add('hidden');
        toggleCameraBtn.innerHTML = '<i class="fas fa-stop"></i> Stop Kamera';
    ...
    function stopCamera() {
        if (html5QrcodeScanner) {
            html5QrcodeScanner.stop().then(() => {
                ...
                toggleCameraBtn.innerHTML = '<i class="fas fa-video"></i> Buka Kamera';
    ...
    // --- Input Manual Listener ---
    tokenInput.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            processToken(this.value.trim());
        }
    });
    ...
    cashPaidInput.addEventListener('input', function() {
        ...
        const change = cash - currentTotalFee;
        
        if (cash >= currentTotalFee) {
            changeDueDisplay.textContent = `Rp ${change.toLocaleString('id-ID')}`;
            changeDueDisplay.className = 'font-bold text-lg text-blue-600';
            submitButton.disabled = false;
            submitButton.classList.remove('opacity-50', 'cursor-not-allowed');
        } else {
            const kurang = Math.abs(change);
            changeDueDisplay.textContent = `Kurang Rp ${kurang.toLocaleString('id-ID')}`;
            changeDueDisplay.className = 'font-bold text-lg text-red-500';
            submitButton.disabled = true;
