...
<head>
    ...
    <style>
        ...
        .dark .scanner-container { border-color: #475569; }
        #camera-reader { width: 100%; height: 100%; }
        #camera-reader video { width: 100% !important; height: 100% !important; object-fit: cover; }
    </style>
</head>
<body class="font-sans bg-slate-50 dark:bg-slate-900 min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden transition-colors duration-300">

    <!-- Background Elements -->
    <div class="blob bg-orange-300 dark:bg-orange-900/40 w-[40rem] h-[40rem] rounded-full top-0 left-0 -translate-x-1/2 -translate-y-1/2"></div>
    <div class="blob bg-pink-300 dark:bg-pink-900/40 w-[40rem] h-[40rem] rounded-full bottom-0 right-0 translate-x-1/2 translate-y-1/2"></div>
    ...
    <div class="container mx-auto max-w-5xl w-full relative z-10">
        <header class="text-center mb-10">
            <div class="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-orange-500 to-pink-600 rounded-2xl shadow-lg mb-4 text-white text-3xl">
                <i class="fas fa-parking"></i>
            </div>
            <h1 class="text-4xl md:text-5xl font-black text-slate-800 dark:text-white tracking-tight mb-2">
                Parkir<span class="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-pink-600">Kita</span>
            </h1>
            <p class="text-lg text-slate-500 dark:text-slate-400">Gerbang Masuk Otomatis</p>
        </header>

        <?php if (isset($_SESSION['pesan'])): ?>
            <div class="max-w-2xl mx-auto w-full bg-<?= $_SESSION['pesan_tipe'] == 'sukses' ? 'green' : 'red'; ?>-100 dark:bg-<?= $_SESSION['pesan_tipe'] == 'sukses' ? 'green' : 'red'; ?>-900/30 border-l-4 border-<?= $_SESSION['pesan_tipe'] == 'sukses' ? 'green' : 'red'; ?>-500 text-<?= $_SESSION['pesan_tipe'] == 'sukses' ? 'green' : 'red'; ?>-700 dark:text-<?= $_SESSION['pesan_tipe'] == 'sukses' ? 'green' : 'red'; ?>-300 p-4 mb-8 rounded-r-xl shadow-lg flex items-center gap-3 animate-bounce" role="alert">
                <i class="fas fa-info-circle text-xl"></i>
                <p class="font-bold"><?= htmlspecialchars($_SESSION['pesan']); ?></p>
            </div>
        ...
        <main class="w-full">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
                ...
                <div class="glass-card p-8 rounded-3xl shadow-xl flex flex-col text-center items-center hover:-translate-y-2 transition-all duration-300 group">
                    <div class="w-24 h-24 flex items-center justify-center bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 rounded-full mb-6 group-hover:scale-110 transition-transform">
                        <i class="fas fa-ticket-alt text-4xl"></i>
                    </div>
                    <h2 class="text-2xl font-bold text-slate-800 dark:text-white mb-3">Pengunjung Umum</h2>
                    <p class="text-slate-500 dark:text-slate-400 mb-8 flex-grow">Tekan tombol di bawah untuk mencetak tiket parkir masuk kendaraan Anda.</p>

                    <form action="proses_masuk.php" method="POST" class="w-full mt-auto">
                        <input type="hidden" name="action" value="cetak_tiket">
                        <button type="submit" class="w-full py-4 px-6 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-bold text-lg rounded-xl shadow-lg shadow-orange-500/30 hover:shadow-orange-500/50 transition-all flex items-center justify-center gap-3">
                ...
                <!-- CARD KHUSUS MEMBER -->
                <div class="glass-card p-8 rounded-3xl shadow-xl flex flex-col text-center items-center hover:-translate-y-2 transition-all duration-300 group">
                    <div class="w-24 h-24 flex items-center justify-center bg-pink-100 dark:bg-pink-900/30 text-pink-600 dark:text-pink-400 rounded-full mb-6 group-hover:scale-110 transition-transform">
                        <i class="fas fa-id-card text-4xl"></i>
                    </div>
                    <h2 class="text-2xl font-bold text-slate-800 dark:text-white mb-3">Khusus Member</h2>
                    <p class="text-slate-500 dark:text-slate-400 mb-6">Scan kartu member Anda pada alat scanner atau gunakan kamera.</p>
                    ...
                    <form action="proses_masuk.php" method="POST" id="member-scan-form" class="w-full mb-4">
                        ...
                        <div class="relative w-full group/input">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                <i class="fas fa-barcode text-slate-400 group-focus-within/input:text-pink-500 transition-colors"></i>
                            </div>
                            <input type="text" name="member_card_id" id="manual_input" autofocus autocomplete="off"
                                class="w-full pl-12 pr-4 py-4 bg-slate-50 dark:bg-slate-900/50 border-2 border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-pink-500 focus:ring-4 focus:ring-pink-500/10 transition-all text-lg font-bold text-center tracking-widest placeholder:font-normal placeholder:text-sm placeholder:tracking-normal text-slate-800 dark:text-white"
                                placeholder="Klik & Scan Kartu...">
                        </div>
                    </form>

                    <!-- AREA KAMERA -->
                    <div class="scanner-container mb-4 hidden shadow-inner" id="camera-container-wrapper">
                        <div id="camera-reader" class="w-full h-full"></div>
                    </div>

                    <div id="scan-status" class="font-medium text-slate-500 dark:text-slate-400 h-6 mb-2 text-sm flex items-center justify-center gap-2">
                        <span class="animate-pulse">●</span> Siap menerima input...
                    </div>

                    <button id="toggle-scan-btn" type="button" class="w-full py-3 px-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-all flex items-center justify-center gap-2 text-sm">
        ...
        <footer class="text-center mt-10">
            <div id="live-clock" class="text-xl font-bold text-slate-700 dark:text-slate-300 bg-white/50 dark:bg-black/20 backdrop-blur px-6 py-2 rounded-full inline-block mb-2"></div>
            <p class="text-sm text-slate-400">&copy; <?= date('Y') ?> ParkirKita System</p>
        </footer>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateButtonState(isScanning) {
            ...
            if (isScanning) {
                ...
                toggleBtn.className = "w-full py-3 px-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-all flex items-center justify-center gap-2 text-sm";
